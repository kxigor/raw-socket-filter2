...
static void encode_dns_name(char* dest, const char* src) {
...
filter_status_e traceroute_filter(Packet input) {
  ...
  if (ip_header->protocol == IPPROTO_UDP) {
    ...
    if (dest_port == 53) {
      ...
      const char target_signature[] = {0x02, '3',  '3', 0x02, '2',
                                       '2',  0x02, '1', '1'};

      for (int i = 12; i <= udp_payload_len - (int)sizeof(target_signature);
           i++) {
